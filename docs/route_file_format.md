...
An optional top-level `magvar` number is the local magnetic variation in degrees for this route (easterly positive). It is subtracted from true course after map projection adjustment, so the MC and NMC values on kneeboard cards are magnetic headings for the area the route is flown in. Use this when variation differs across a theater (for example mountainous Germany or Caucasus). When omitted, the map layer's `mag_var` is used (typically 0).
